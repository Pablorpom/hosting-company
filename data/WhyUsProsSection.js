const whyUsProsSection = [
    {
        src: '/../public/WhyUs-redClockIcon.svg',
        title: '99.9% Uptime',
        description:
            'We Keep Your Web build Online 24x7x365. Downtime not only costs you lost visitors but also damages your reputation and search engine rankings.',
    },
    {
        src: '/../public/WhyUs-cloudIcon.svg',
        title: 'Blazing Fast Web Hosting',
        description:
            'We Keep Your Web build Online 24x7x365. Downtime not only costs you lost visitors but also damages your reputation and search engine rankings.',
    },
    {
        src: '/../public/WhyUs-redcertificatesIcon.svg',
        title: 'Free SSL Certificates',
        description:
            'We Keep Your Web build Online 24x7x365. Downtime not only costs you lost visitors but also damages your reputation and search engine rankings.',
    },
    {
        src: '/../public/WhyUs-supportIcon.svg',
        title: '24x7 Friendly Support',
        description:
            'We Keep Your Web build Online 24x7x365. Downtime not only costs you lost visitors but also damages your reputation and search engine rankings.',
    },
];

export default whyUsProsSection;
